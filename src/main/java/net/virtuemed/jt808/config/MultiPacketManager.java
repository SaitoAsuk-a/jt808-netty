...
import net.virtuemed.jt808.vo.Header;
...
public enum MultiPacketManager {
    ...
    public byte[][] addAndGet(Header header, byte[] packetData) {
        String clientId = header.getTerminalPhone();
        int messageId = header.getMsgId();
        int packageTotal = header.getPacketsTotalNumber();
        int packetNo = header.getPacketSequenceNumber();

        String key = new StringBuilder(21)
                .append(clientId)
                .append("/")
                .append(messageId)
                .append("/")
        ...
        if (packetNo == 1)
            multiPacket.setSerialNo(header.getFlowId());
